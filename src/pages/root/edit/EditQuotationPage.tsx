...
import { addQuotation, editQuotation, getQuotationById } from "@/db/Quotations";
import { Customer, QuotationStatus } from "@/API";
import CustomerSelector from "@/components/shared/CustomerSelector";
import generatePDF from "react-to-pdf";
import { updateQuotation } from "@/graphql/mutations";
...
export default function UpdateQuotationPage() {
  ...
  const onSubmit = async (data: QuotationFormValues) => {
    try {
      const newQuotation = await editQuotation({
        id: params.id!!,
  ...
  const fetchQuotation = async () => {
    try {
      ...
      if (quotationData) {
        form.setValue("quotationNumber", quotationData.quotation_number);
        form.setValue("quotationDate", new Date(quotationData.quotation_date));
        form.setValue("validUntil", new Date(quotationData.valid_until));
        form.setValue("fromCompany", quotationData.from_company);
        form.setValue("fromAddress", quotationData.from_address);
        form.setValue("fromGstin", quotationData.from_gstin);
        form.setValue("fromEmail", quotationData.from_email);
        form.setValue("fromPhone", quotationData.from_phone);
        form.setValue("toCustomer", quotationData.to_customer);
        form.setValue("toAddress", quotationData.to_address);
        form.setValue("toGstin", quotationData.to_gstin);
        form.setValue("toEmail", quotationData.to_email);
        form.setValue("toPhone", quotationData.to_phone);
        form.setValue("items", JSON.parse(quotationData.items));
        form.setValue("notes", quotationData.notes);
        form.setValue("termsAndConditions", quotationData.terms_conditions);
        form.setValue("subtotal", Number(quotationData.subtotal));
        form.setValue("cgst", Number(quotationData.cgst));
        form.setValue("sgst", Number(quotationData.sgst));
        form.setValue("igst", Number(quotationData.igst));
        form.setValue("total", Number(quotationData.total));
        form.setValue("quotationStatus", quotationData.quotation_status);
      }
    } catch (error) {
